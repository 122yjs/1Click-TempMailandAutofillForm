...
export function useInboxActions(state: InboxActionsState, config: InboxActionsConfig = {}) {
  ...
  async function selectAccount(address: string) {
    // Snapshot live lists at call time (getters) so prev/next always resolves the account
    const accountsSnap = state.accounts;
    const allInboxesSnap = state.allInboxes;
    await selectAccountAction(
      ext,
      address,
      {
        accounts: accountsSnap,
        allInboxes: allInboxesSnap,
        emails: [],
