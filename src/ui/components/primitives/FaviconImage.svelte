...
let {
  ...
  class: className = '',
  fallbackLetter = '?',
  /** Explicit override; when empty a Google-style color is hashed from email/domain. */
  fallbackColor = '',
...
/** Fallback shell: explicit override wins, otherwise a Google-style color. */
let resolvedFallbackColor = $derived(
  fallbackColor || avatarColor(email || resolvedDomain || fallbackLetter)
);
...
>
  <!-- Letter avatar: same slot, same z-index, same radius as the favicon.
       Rendered only until a real icon loads, then removed - the favicon
       replaces it instead of stacking on top. -->
  {#if !loaded}
    <span
      class="absolute inset-0 flex items-center justify-center font-bold select-none text-white {resolvedFallbackColor}"
      style="font-size: {Math.max(8, Math.round(size * 0.52))}px; line-height: 1;"
      aria-hidden="true"
    >
  ...
  {#if enabled && !hasRecentErr && attempt !== 'failed' && attempt !== 'idle'}
    {#if (attempt === 'cached' || revalidating) && cachedUrl}
      <img
        src={cachedUrl}
        alt=""
        class="relative w-full h-full object-cover {loaded ? 'opacity-100' : 'opacity-0'}"
        onload={handleLoad}
        onerror={() => {
