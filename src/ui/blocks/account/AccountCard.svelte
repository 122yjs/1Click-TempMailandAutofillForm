...
const allowUnarchive = $derived(canUnarchive(account));
...
<div
  ...
  ondrop={onDrop}
>
    <!-- 1st column: letter avatar (popup only) -->
    <div
      class="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center {avatarColor(
        account.address
      )} {selectedEmail === account.address ? 'ring-2 ring-md-primary ring-offset-1 ring-offset-md-surface' : ''}"
      aria-hidden="true"
    >
      <span class="text-sm font-bold uppercase select-none">{(account.address[0] || '?')}</span>
    </div>
    ...
    <div class="relative shrink-0 self-center" style="z-index: {menuOpen ? 120 : 1};">
      ...
      {#if menuOpen}
        ...
        ></button>
        <MenuList
          class="fixed {PORTAL_Z_CLASS.accountMenu} min-w-[148px]"
          role="menu"
        ...
        >
          {#if isArchivedAccount}
            <button
              ...
              role="menuitem"
              class="menu-list-item w-full flex items-center gap-3 px-3 min-h-12 text-sm text-start {allowUnarchive ? '' : 'is-disabled'}"
              disabled={!allowUnarchive}
              aria-disabled={!allowUnarchive}
              title={allowUnarchive
                ? $t('common.unarchive')
                : $t('account.unarchiveNotAvailable')}
              onclick={(e) => {
                e.stopPropagation();
                if (!allowUnarchive) return;
                closeMenu();
          ...
          {:else if !isDeletedAccount}
            <button
              ...
              role="menuitem"
              class="menu-list-item w-full flex items-center gap-3 px-3 min-h-12 text-sm text-start"
              onclick={(e) => {
                e.stopPropagation();
                closeMenu();
          ...
          {:else}
            <button
              ...
              role="menuitem"
              class="menu-list-item w-full flex items-center gap-3 px-3 min-h-12 text-sm text-start is-danger"
              onclick={(e) => {
                e.stopPropagation();
                closeMenu();
          ...
          <button
            type="button"
            role="menuitem"
            class="menu-list-item w-full flex items-center gap-3 px-3 min-h-12 text-sm text-start {liveCanMarkAllRead ? '' : 'is-disabled'}"
          ...
          >
            ...
            {$t('addressView.markAllRead')}
          </button>
          <button
            type="button"
            role="menuitem"
            class="menu-list-item w-full flex items-center gap-3 px-3 min-h-12 text-sm text-start {liveCanMarkAllUnread ? '' : 'is-disabled'}"
