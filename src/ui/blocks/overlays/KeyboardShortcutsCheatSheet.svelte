...
const shortcutLabels: Record<string, string> = {
  refreshInbox: 'keyboardShortcuts.refreshInbox',
  createInbox: 'keyboardShortcuts.createInbox',
  copyEmail: 'keyboardShortcuts.copyEmail',
  copyOtp: 'keyboardShortcuts.copyOtp',
  closeDialogs: 'keyboardShortcuts.closeDialogs',
  openAddresses: 'keyboardShortcuts.openAddresses',
  openIdentities: 'keyboardShortcuts.openIdentities',
  openSavedLogins: 'keyboardShortcuts.openSavedLogins',
  toggleAccountSelector: 'keyboardShortcuts.toggleAccountSelector',
...
const fixedShortcuts: Array<{ label: string; keys: string[] }> = [
  { label: 'keyboardShortcuts.fixedFocusSearch', keys: ['/'] },
  { label: 'keyboardShortcuts.fixedNextMessage', keys: ['J'] },
  { label: 'keyboardShortcuts.fixedPreviousMessage', keys: ['K'] },
  { label: 'keyboardShortcuts.fixedCommandPalette', keys: [isMac ? '⌘' : 'Ctrl', 'K'] },
];
...
{#if open}
  ...
  >
    ...
    <div
      ...
      role="button"
      tabindex="-1"
      aria-label={$t('keyboardShortcuts.dismissSheetAria')}
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-md-outline-variant/20 pb-3">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h2 id="cheat-sheet-title" class="text-sm font-bold tracking-tight">{$t('keyboardShortcuts.title')}</h2>
            <p class="text-xs text-md-on-surface/50">{$t('keyboardShortcuts.cheatSheetSubtitle')}</p>
          </div>
        </div>
        <button
          class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-md-surface-variant/50 transition-colors"
          onclick={(e) => { e.stopPropagation(); onClose(); }}
          aria-label={$t('keyboardShortcuts.closeSheetAria')}
        >
      ...
      <div class="space-y-2.5">
        {#each Object.entries(shortcutLabels) as [key, label]}
          ...
          <div class="flex items-center justify-between p-2 rounded-xl bg-md-surface-container-low/60 border border-md-outline-variant/10">
            <span class="text-xs font-medium text-md-on-surface/80">{$t(label)}</span>
        ...
        {#each fixedShortcuts as row (row.label)}
          <div class="flex items-center justify-between p-2 rounded-xl bg-md-surface-container-low/60 border border-md-outline-variant/10">
            <span class="text-xs font-medium text-md-on-surface/80">{$t(row.label)}</span>
            <div class="flex items-center gap-1">
        ...
        <div class="flex items-center justify-between p-2 rounded-xl bg-md-primary/5 border border-md-primary/15">
          <span class="text-xs font-medium text-md-primary/90">{$t('keyboardShortcuts.toggleCheatSheet')}</span>
      ...
      <div class="text-center pt-1 border-t border-md-outline-variant/20">
        <span class="text-xs text-md-on-surface/40">{$t('keyboardShortcuts.dismissPress')} <kbd class="px-1 py-0.5 text-xs font-mono bg-md-surface-variant rounded">Esc</kbd> {$t('keyboardShortcuts.dismissOr')} <kbd class="px-1 py-0.5 text-xs font-mono bg-md-surface-variant rounded">?</kbd> {$t('keyboardShortcuts.dismissDismiss')}</span>
      </div>
    </div>
  </div>
{/if}
