...
$effect(() => {
  void filtered;
  activeIdx = filtered.length > 0 ? Math.min(activeIdx, filtered.length - 1) : 0;
});
    ...
    >
      <div class="flex items-center gap-2 px-3 py-2.5 border-b border-md-outline-variant/20">
        ...
        <input
          ...
          autocomplete="off"
        />
        <kbd class="text-label-sm font-mono px-1.5 py-0.5 rounded bg-md-surface-variant text-md-on-surface/50"
          >Esc</kbd
        >
      </div>
