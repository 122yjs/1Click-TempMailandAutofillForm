...
import { Checkbox, Radio } from '@/ui/components/primitives';
...
async function pasteFromClipboard() {
  ...
  try {
    ...
    try {
      ...
      text = window.prompt($t('backup.pastePrompt'), '') || '';
    }
    if (!text?.trim()) {
...
// Clean up unused overlayEl and cleanupFocusTrap logic handled by ModalDialog
</script>

<ModalDialog
  {open}
  title={$t('backup.importTitle')}
  subtitle={$t('backup.importSubtitle')}
  maxWidth="sm"
          ...
          {#if selected.inboxes && presentCategories.includes('inboxes')}
            <div class="ms-0.5 ps-3 border-s-2 border-md-primary/30 space-y-2">
              ...
              {#each [
                ...
                <label class="flex items-start gap-2 text-xs cursor-pointer">
                  <Radio
                    checked={expiredMode === opt.value}
                    onchange={() => {
                      expiredMode = opt.value;
                      void refreshDryRun();
                    }}
                  />
                  <span>
