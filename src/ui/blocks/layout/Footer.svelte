...
let {
  ...
  /** Gear on provider menu → mail provider settings */
  onOpenProviderSettings?: () => void;
  /** Long-press floating navbar → open Customize Navigation Bar */
  onLongPressNavbar?: () => void;
...
function startNavbarLongPress(e: PointerEvent) {
  ...
  longPressTimer = setTimeout(() => {
    ...
    if (onLongPressNavbar) {
      onLongPressNavbar();
    }
  }, 500);
}

function cancelNavbarLongPress() {
  if (longPressTimer) {
...
function checkNavbarPointerMove(e: PointerEvent) {
  if (!longPressTimer) return;
  const dx = Math.abs(e.clientX - pointerStartX);
  const dy = Math.abs(e.clientY - pointerStartY);
  if (dx > 10 || dy > 10) {
    cancelNavbarLongPress();
  }
}
...
let customNavOrder = $state<string[]>([]);

onMount(() => {
  void browser.storage.local.get(['customNavOrder']).then((res: Record<string, unknown>) => {
    if (res.customNavOrder && Array.isArray(res.customNavOrder)) {
      customNavOrder = res.customNavOrder as string[];
    }
  });
  const listener = (changes: Record<string, { newValue?: unknown }>, area: string) => {
    if (area === 'local' && changes.customNavOrder) {
      customNavOrder = (changes.customNavOrder.newValue as string[]) || [];
    }
  };
  try {
...
let activeNavLayout = $derived.by(() => {
  const defaultOrder = [
    'mailbox',
    'addresses',
    'autofill',
  ...
  const userOrder = customNavOrder.length ? customNavOrder : defaultOrder;
  const orderedKeys = userOrder.filter((k) => defaultOrder.includes(k));
  const missing = defaultOrder.filter((k) => !orderedKeys.includes(k));
  const fullOrder = [...orderedKeys, ...missing];

  const mapItem = (key: string) => {
    switch (key) {
      case 'addresses':
        return { view: 'addresses' as View, labelKey: 'nav.addresses', iconName: 'envelope' };
      case 'autofill':
        return { view: 'autofill' as View, labelKey: 'nav.autofill', iconName: 'person' };
      case 'organize':
        return { view: 'organize' as View, labelKey: 'nav.organize', iconName: 'grid' };
      case 'automation':
        return { view: 'automation' as View, labelKey: 'nav.automation', iconName: 'flame' };
      case 'activity':
        return { view: 'analytics' as View, labelKey: 'nav.activity', iconName: 'barChart' };
      case 'settings':
        return { view: 'settings' as View, labelKey: 'nav.settings', iconName: 'settings' };
      case 'about':
        return { view: 'about' as View, labelKey: 'nav.about', iconName: 'info' };
      default:
        return { view: 'mailbox' as View, labelKey: 'nav.mailbox', iconName: 'inbox' };
    }
  };

  const countVisible =
    navStage === 'seven' ? 7 : navStage === 'six' ? 5 : navStage === 'plus_organize' ? 4 : 3;

  if (navStage === 'seven') {
    return {
      primaryNav: fullOrder.map(mapItem) as NavItem[],
      moreItems: [] as MoreItem[],
    };
  }

  const primaryKeys = fullOrder.slice(0, countVisible);
  const overflowKeys = fullOrder.slice(countVisible);

  const primaryNav: NavItem[] = [
    ...primaryKeys.map(mapItem),
    { view: 'more', labelKey: 'nav.more', iconName: 'moreHorizontal' },
  ];

  const moreItems: MoreItem[] = overflowKeys.map(mapItem) as MoreItem[];

  return { primaryNav, moreItems };
});

let primaryNav = $derived(activeNavLayout.primaryNav);
let moreItems = $derived(activeNavLayout.moreItems);
...
>
  ...
  {#if moreOpen && moreItems.length > 0}
    ...
    <button
      type="button"
      class="pointer-events-auto fixed inset-0 {PORTAL_Z_CLASS.navMenu} bg-transparent cursor-default"
      aria-label={$t('common.close')}
      onclick={() => closeMoreNow()}
    ></button>
    <div
      class="pointer-events-auto absolute bottom-full mb-2 left-1/2 -translate-x-1/2 {PORTAL_Z_CLASS.accountMenu} more-menu-nav"
      role="menu"
      tabindex="-1"
  ...
  <div class="bottom-nav-container pointer-events-auto flex items-stretch justify-center gap-1.5 w-full px-0.5">
    <nav
      ...
      style="--nav-font-size: {navFontPx}px;"
      onpointerdown={startNavbarLongPress}
      onpointermove={checkNavbarPointerMove}
      onpointerup={cancelNavbarLongPress}
      onpointercancel={cancelNavbarLongPress}
    >
      ...
      {#each primaryNav as item (item.view)}
        ...
        {@const isMailbox = item.view === 'mailbox'}
        {@const label = $t(item.labelKey)}
        <button
          type="button"
          class="nav-item {isActive ? 'active' : ''} {isMailbox && totalUnread > 0 ? 'has-badge' : ''}"
...
<style>
  ...
  .floating-nav {
    display: flex;
    align-items: center;
    /* iOS liquid glass: very translucent with heavy blur + saturation boost */
    background-color: color-mix(in srgb, var(--md-surface-container-high) 55%, transparent);
    ...
    box-shadow:
      0 2px 24px color-mix(in srgb, var(--md-shadow, #000) 18%, transparent),
      0 1px 0 color-mix(in srgb, var(--md-inverse-surface) 10%, transparent) inset,
      0 0 0 0.5px color-mix(in srgb, var(--md-outline-variant) 20%, transparent) inset;
  }
  ...
  /* More overflow menu — pill island matching floating-nav.
     Wraps onto a second row when items don't fit one line (popup shows 5
     items → 2 rows); wider shells promote items to the primary nav, so
     fewer items naturally collapse back to a single row. */
  .more-menu-nav {
    display: flex;
    align-items: center;
    flex-wrap: wrap;
    ...
    padding: 4px;
    border-radius: 36px;
    background-color: color-mix(in srgb, var(--md-surface-container-high) 60%, transparent);
    backdrop-filter: blur(28px) saturate(1.8) brightness(1.04);
    -webkit-backdrop-filter: blur(28px) saturate(1.8) brightness(1.04);
    border: 1px solid color-mix(in srgb, var(--md-outline-variant) 30%, transparent);
    box-shadow:
